...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="startAtLineBeginning" as="xs:boolean" select="false()"/>
    ...
    <!-- List of special characters:
         If 'insitu' then special characters are assumed to be defined in the to-be-expanded TEI file. -->
    <xsl:param name="specialCharFile" as="xs:string" select="'../../../resources/chars/specialchars_2019-04-03.xml'"/>
    
    <!-- The usual stuff: -->
    <xsl:param name="editors" as="xs:string" select="'#DG #auto'"/>
    <xsl:param name="editingDate" as="xs:string" select="'YYYY-MM-DD'"/>
    <xsl:param name="changeId" as="xs:string" select="'Wxxxx_change_yyy'"></xsl:param>
    ...
    <xsl:variable name="wordDelimiters" as="xs:string" select="'[,.: \?!\n()]'"/> 
    ...
    <xsl:variable name="specialCharacters" as="element(tei:char)+" select="doc(if ($specialCharFile eq 'insitu') then base-uri() else $specialCharFile)//tei:teiHeader//tei:charDecl//tei:char"/>
    <xsl:variable name="sanitizeInput">
        <xsl:if test="not($lang = ('es', 'la', 'pt')) or not($mode = ('main', 'marginal'))">
            <xsl:message terminate="yes" select="'ERROR: illegal input values for parameters $lang (must be one of -es-, -pt- or -la-) or $mode (must be one of -main- or -marginal-)'"/>
        </xsl:if>
    </xsl:variable>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:message terminate="yes" select="concat('ERROR: found more than two or three abbrExpan items for abbrExpan/@type ', $actualAbbr[1], ': ', 
                                    count($actualAbbr), ' elements in total ( ', string-join($actualAbbr, ' | '), ' ).', ' Perhaps an error in the abbrExpan list?')"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:otherwise>
                </xsl:choose>
    ...
    <xsl:function name="local:isDelimitedLeft" as="xs:boolean">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$startAtLineBeginning">
                <xsl:choose>
                    <xsl:when test="$node/preceding-sibling::node()[1]/self::*[local-name() = $delimiterElems]">
                        <xsl:value-of select="true()"/>
                    </xsl:when>
    ...
    <!-- 3.) quality check, logging, and output -->
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="choiceDiff" as="xs:integer" select="count($expandedAbbr//tei:choice[tei:expan]) - count($identifiedAbbr//tei:choice[tei:expan])"/>
        <xsl:message select="concat('INFO: added ', xs:string($choiceDiff), ' choice/expan in total.')"/>
        
        <!-- final output: -->
        <xsl:copy-of select="$expandedAbbr"/>
    </xsl:template>
